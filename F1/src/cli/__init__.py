...
from src.modeling.train import run_ensemble_backtest, run_agent_backtest, compare_agent_with_strategies, \
    main as train_main
...
def run_backtest_only(args):
    ...
    from src.modeling.ppo import PPOAgent
    ...
    _, _, _, _, _, _, df_normalized = prepare_data(
        ...
        load_path=args.data_path if args.load_timeseries and os.path.exists(args.data_path) else None
    )

    # Load ensemble models
    model_paths = {
        'lstm': args.lstm_models_path,
        'gru': args.gru_models_path,
        'cnn': args.cnn_models_path,
        'mlp': args.mlp_models_path
    }
    ...
    # Only load the PPO agent if use_ppo is True
    if args.use_ppo and os.path.exists(args.agent_models_path):
        print(f"Loading PPO agent from {args.agent_models_path}...")
        ...
        # Load state dict
        agent.load_state_dict(torch.load(args.agent_models_path))
        agent.eval()
...
def parse_args():
    ...
    parser.add_argument('--days', type=int, default=1,
                        help='Number of days of timeseries to fetch (default: 1)')
    parser.add_argument('--seq-length', type=int, default=60,
                        help='Sequence length for time series models (default: 60)')
    parser.add_argument('--timeseries-path', type=str, default='timeseries/raw/btc_usdt_1s.csv',
                        help='Path to save/load timeseries (default: timeseries/raw/btc_usdt_1s.csv)')
    parser.add_argument('--load-timeseries', action='store_true',
                        help='Load timeseries from file if available (default: True)')
    parser.add_argument('--save-timeseries', action='store_true',
                        help='Save timeseries to file (default: True)')
    ...
    args.data_path = args.timeseries_path
